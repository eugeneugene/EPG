...
namespace EPG.Code
{
    public class EPGConfiguratorSettingsProvider : LocalFileSettingsProvider
    {
        ...
        private static string GetSectionName(SettingsContext context)
        {
            ...
            var section = context ["GroupName"];
            if (section is not null)
                return section.ToString() ?? string.Empty;
            return string.Empty;
        }
        ...
        private static void WriteSettingsToFile(string path, string sectionName, SettingsPropertyValueCollection values)
        {
            try
            {
                if (!string.IsNullOrEmpty(path) && !string.IsNullOrEmpty(sectionName))
                {
                    System.Configuration.Configuration configuration = GetConfiguration(path);
